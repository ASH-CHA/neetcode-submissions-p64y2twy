...
class Solution {
    public List<Pair> quickSort(List<Pair> pairs) {
        ...
        quickSortHelp(pairs, 0, pairs.size() - 1);
        return pairs;
    }

    private void quickSortHelp(List<Pair> pairs, int low, int high){
        if(low < high){
            int pivotIndex = partition(pairs, low, high);
            quickSortHelp(pairs, low, pivotIndex - 1);
            quickSortHelp(pairs, pivotIndex + 1, high);
        }
    }
